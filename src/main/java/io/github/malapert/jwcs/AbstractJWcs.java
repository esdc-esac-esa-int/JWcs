...
import io.github.malapert.jwcs.datum.*;
import io.github.malapert.jwcs.proj.*;
...
public abstract class AbstractJWcs implements JWcsKeyProvider {
    ...
    protected final void init() throws JWcsException {
        ...
        setCdInverse(inverse(getCd()).transpose());
    }
     ...
     */
    protected static double[][] computeCdFromCdelt(final double[] cdelt, final double crota) {
    ...
    public boolean hasCd() {
        return hasKeyword(CD11) || (hasKeyword(CDELT1) && hasKeyword(CROTA2)) ||
    ...
    protected final AbstractProjection createProjection() throws BadProjectionParameterException {
        ...
        final String codeProjection = ctype1.substring(ctype1.lastIndexOf('-') + 1);
     ...
     */
    private AbstractProjection createProjectionFactory(final String projectionCode, final double cx, final double cy)
        ...
        case "TPV":
            ...
            break;
        case "BON":
            LOG.log(Level.INFO, "Creates a AIT projection with (crval1,crval2)=({0},{1}) theta1={2}",
                    new Object[] { crval(1) * cx, crval(2) * cx, getValueAsDouble(PV21, 0) });
            projection = new BON(crval(1) * cx, crval(2) * cy, getValueAsDouble(PV21, 0));
            break;
        case "CEA":
            LOG.log(Level.INFO, "Creates a CEA projection with (crval1,crval2)=({0},{1}) lambda={2}",
                    new Object[] { crval(1) * cx, crval(2) * cx, getValueAsDouble(PV21, 0) });
            projection = new CEA(crval(1) * cx, crval(2) * cy, getValueAsDouble(PV21, 1));
            break;
        case "COD":
            LOG.log(Level.INFO, "Creates a COD projection with (crval1,crval2)=({0},{1}) (theta_a,eta)=({2},{3})",
                    new Object[] { crval(1) * cx, crval(2) * cx, getValueAsDouble(PV21, 0),
                                   getValueAsDouble(PV22, 0) });
            projection = new COD(crval(1) * cx, crval(2) * cy, getValueAsDouble(PV21, 0), getValueAsDouble(PV22, 0));
            break;
        case "COE":
            LOG.log(Level.INFO, "Creates a COE projection with (crval1,crval2)=({0},{1}) (theta_a,eta)=({2},{3})",
                    new Object[] { crval(1) * cx, crval(2) * cx, getValueAsDouble(PV21, 0),
                                   getValueAsDouble(PV22, 0) });
            projection = new COE(crval(1) * cx, crval(2) * cy, getValueAsDouble(PV21, 0), getValueAsDouble(PV22, 0));
            break;
        case "COO":
            LOG.log(Level.INFO, "Creates a COO projection with (crval1,crval2)=({0},{1}) (theta_a,eta)=({2},{3})",
                    new Object[] { crval(1) * cx, crval(2) * cx, getValueAsDouble(PV21, 0),
                                   getValueAsDouble(PV22, 0) });
            projection = new COO(crval(1) * cx, crval(2) * cy, getValueAsDouble(PV21, 0), getValueAsDouble(PV22, 0));
            break;
        case "COP":
            LOG.log(Level.INFO, "Creates a COP projection with (crval1,crval2)=({0},{1}) (theta_a,eta)=({2},{3})",
                    new Object[] { crval(1) * cx, crval(2) * cx, getValueAsDouble(PV21, 0),
                                   getValueAsDouble(PV22, 0) });
        ...
        case "SZP":
            ...
            break;
        case "NCP":
            LOG.log(Level.INFO, "Creates a NCP projection with (crval1,crval2)=({0},{1}) (ksi,eta)=({2},{3})",
                    new Object[] { crval(1) * cx, crval(2) * cx, 0, 1 / FastMath.tan(getValueAsDouble(CRVAL2)) });
    ...
    private AbstractProjection createStandardProjection(final String projectionCode, final double cx, final double cy) {
        ...
        try {
            ...
            LOG.log(Level.INFO, "Creates a {0} projection with (crval1,crval2)=({1},{2})",
                    new Object[] { projectionCode, crval(1) * cx, crval(2) * cx });
     ...
     * dynamically the projection
     */
    private AbstractProjection createStandardProjectionWithParameters(final String projectionCode,
                                                                      final double cx,
                                                                      final double cy) {
        final AbstractProjection projection;
        final Package packageName = this.getClass().getPackage();
        ...
        try {
            ...
            if (hasKeyword(PV22)) {
                ...
                LOG.log(Level.INFO, "Creates a {0} projection with (crval1,crval2)=({1},{2}) (pv21,pv22)=({3},{4})",
                        new Object[] { projectionCode, crval(1) * cx, crval(2) * cx, pv21, pv22 });
            } else {
                ...
                LOG.log(Level.INFO, "Creates a {0} projection with (crval1,crval2)=({1},{2}) pv21={3}",
                        new Object[] { projectionCode, crval(1) * cx, crval(2) * cx, pv21 });
            }
            projection = (AbstractProjection)instance;
        } catch (ClassNotFoundException ex) {
            throw new JWcsError("The projection " + projectionCode + " is not supported.");
     ...
     * to the projection
     */
    private AbstractProjection createSZPProjection(final double cx, final double cy)
            throws BadProjectionParameterException {
        final AbstractProjection projection;
        if (hasKeyword(PV21) && hasKeyword(PV22) && hasKeyword(PV23)) {
            LOG.log(Level.INFO,
                    "Creates a SZP projection with (crval1,crval2)=({0},{1}) (mu,phic,thetac)=({2},{3},{4})",
                    new Object[] { crval(1) * cx, crval(2) * cx, getValueAsDouble(PV21), getValueAsDouble(PV22),
                                   getValueAsDouble(PV23) });
            projection = new SZP(crval(1) * cx, crval(2) * cy, getValueAsDouble(PV21), getValueAsDouble(PV22),
                                 getValueAsDouble(PV23));
        } else {
    ...
    private AbstractProjection createZPNProjection(final double cx, final double cy) {
        ...
        LOG.log(Level.INFO, "Creates a ZPN projection with (crval1,crval2)=({0},{1} PV={2})",
                new Object[] { crval(1) * cx, crval(2) * cx, Arrays.toString(pvsPrimitif) });
     ...
     * to the projection
     */
    private AbstractProjection createTPVProjection(final double cx, final double cy)
            throws BadProjectionParameterException {
        final Iterator<Object> iter = iterator();
        ...
        LOG.log(Level.INFO, "Creates a TPV projection with (crval1,crval2)=({0},{1} PV_a={2} PV_b={3})",
                new Object[] { crval(1) * cx, crval(2) * cx, Arrays.toString(pv_a), Arrays.toString(pv_b) });
    ...
    public double[] pix2wcs(final double x, final double y) throws ProjectionException {
        final double[][] arraypj = { { x - crpix(1), y - crpix(2) } };
    ...
    public double[] getFov() throws ProjectionException {
        return pix2wcs(
                new double[] { 0.5, 0.5, naxis(1) + 0.5, 0.5, naxis(1) + 0.5, naxis(2) + 0.5, 0.5, naxis(2) + 0.5 });
    }
    ...
    public boolean isLineToDraw(final double[] pos1, final double[] pos2) {
        ...
        final boolean isFinite = Double.isFinite(pos1[0]) && Double.isFinite(pos1[1]) && Double.isFinite(pos2[0]) &&
                                 Double.isFinite(pos2[1]);
        if (isFinite) {
    ...
    public double[] wcs2pix(final double longitude, final double latitude) throws ProjectionException {
        ...
        final double[] coordVal =
                this.getProj().wcs2projectionPlane(FastMath.toRadians(longitude), FastMath.toRadians(latitude));
        final double[][] coord = { { coordVal[0], coordVal[1] } };
